...
public interface IDataHolder<T>
{
	...
	default void setData(ItemStack stack, T value)
	{
		if(this instanceof Property property)
		{
			PropertyDataComponent.Mutable data = new PropertyDataComponent.Mutable(stack.getOrDefault(AlchemancyItems.Components.PROPERTY_DATA, PropertyDataComponent.EMPTY));
			data.setDataNbt(AlchemancyProperties.getHolder(property), writeData(value));
			stack.set(AlchemancyItems.Components.PROPERTY_DATA, data.toImmutable());
		}
	}
}
